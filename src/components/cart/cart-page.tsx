...
export function CartPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-5 py-10 lg:px-8">
      <SmartBackButton fallbackHref="/shop" fallbackLabel="Browse products" />

      <div className="mt-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-gold">Your selections</p>
          <h1 className="mt-2 text-4xl font-semibold text-forest-green">Your cart</h1>
          {items.length > 0 && <p className="mt-2 text-sm text-muted">Everything is saved and ready when you check out.</p>}
        </div>
      ...
      {items.length === 0 ? (
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_360px]">
          <section aria-label="Cart items" className="space-y-1">
            {items.map((item) => (
